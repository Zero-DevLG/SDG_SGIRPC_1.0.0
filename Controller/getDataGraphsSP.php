...
$json5 = array();
for ($j = 0; $j < $days; $j++) {
    $date_data = $pdo->prepare("SELECT COUNT(de.id_documento) FROM documentos_externos as de WHERE fecha_recibido = '$next_date[$j]' ");
    $date_data->execute();
    ...
    $data_response = $pdo->prepare("SELECT COUNT(id_documento) FROM documentos_atendidos as da WHERE da.date_response = '$next_date[$j]'");
